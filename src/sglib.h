...
#define SGLIB_QUEUE_ADD_NEXT(type, a, i, j, dim) {\
  if (SGLIB_QUEUE_IS_FULL(type, a, i, j, dim)) SGLIB_ASSERT(0 && "the queue is full");\
  (j) = ((j)+1) % (dim);\
}
...
#define SGLIB_QUEUE_DELETE_FIRST(type, a, i, j, dim) {\
  if (SGLIB_QUEUE_IS_EMPTY(type, a, i, j)) SGLIB_ASSERT(0 && "the queue is empty");\
  (i) = ((i)+1) % (dim);\
}
...
#define SGLIB_HEAP_ADD_NEXT(type, a, i, dim, comparator, elem_exchanger) {\
  int _i_;\
  if (SGLIB_HEAP_IS_FULL(type, a, i, dim)) SGLIB_ASSERT(0 && "the heap is full");\
  _i_ = (i)++;\
...
#define SGLIB_HEAP_DELETE_FIRST(type, a, i, dim, comparator, elem_exchanger) {\
  if (SGLIB_HEAP_IS_EMPTY(type, a, i)) SGLIB_ASSERT(0 && "the heap is empty");\
  (i)--;\
  a[0] = a[i];\
...
#define SGLIB_HASH_TAB_ADD_IF_NOT_MEMBER(type, table, dim, elem, hash_function, comparator, member){\
  ...
  (member) = (table)[_pos_];\
  if (_elem_ == NULL) {\
    if ((table)[_pos_] != NULL) SGLIB_ASSERT(0 && "the hash table is full");\
    (table)[_pos_] = (elem);\
  }\
}
...
#define SGLIB_LIST_DELETE(type, list, elem, next) {\
  ...
  SGLIB_ASSERT(*_p_!=NULL && "element is not member of the container, use DELETE_IF_MEMBER instead"!=NULL);\
  *_p_ = (*_p_)->next;\
}
...
#define SGLIB___BIN_TREE_MAP_ON_ELEMENTS(type, tree, iteratedVariable, order, left, right, command) {\
  ...
  while (_cn_!=NULL) {\
    ...
    while(_cn_!=NULL) {\
      ...
      _pathi_ ++;\
      if (_pathi_ >= SGLIB_MAX_TREE_DEEP) SGLIB_ASSERT(0 && "the binary_tree is too deep");\
    }\
    do {\
      _pathi_ --;\
...
#define SGLIB___RBTREE_FIX_DELETION_DISCREPANCY(type, tree, leftt, rightt, bits, RED, BLACK, res) {\
  ...
  b = t->leftt;\
  if (b==NULL) {\
    SGLIB_ASSERT(SGLIB___GET_VALUE(t->bits)==RED);\
    ...
    if (SGLIB___GET_VALUE(b->bits)==RED) {\
      if (br==NULL) {\
        *tree = b;\
        ...
        c = br;\
        SGLIB_ASSERT(c!=NULL && SGLIB___GET_VALUE(c->bits)==BLACK);\
        cl = c->leftt;\
        cr = c->rightt;\
        if ((cl==NULL||SGLIB___GET_VALUE(cl->bits)==BLACK) && (cr==NULL||SGLIB___GET_VALUE(cr->bits)==BLACK)) {\
          *tree = b;\
          ...
          SGLIB_ASSERT(cl==NULL || SGLIB___GET_VALUE(cl->bits)==BLACK);\
          d = cr;\
          dl = d->leftt;\
          dr = d->rightt;\
          *tree = d;\
      ...
      if ((bl==NULL || SGLIB___GET_VALUE(bl->bits)==BLACK) && (br==NULL || SGLIB___GET_VALUE(br->bits)==BLACK)) {\
        ...
        if (br==NULL || SGLIB___GET_VALUE(br->bits)==BLACK) {\
          *tree = b;\
          ...
          SGLIB_ASSERT(bl!=NULL);\
          SGLIB_ASSERT(br!=NULL);\
          SGLIB_ASSERT(SGLIB___GET_VALUE(bl->bits)==RED);\
          SGLIB_ASSERT(SGLIB___GET_VALUE(br->bits)==RED);\
          c = br;\
          cl = c->leftt;\
          cr = c->rightt;\
          *tree = c;\
        ...
        SGLIB_ASSERT(br!=NULL && SGLIB___GET_VALUE(br->bits)==RED);\
        c = br;\
        cl = c->leftt;\
        cr = c->rightt;\
        *tree = c;\
...
int sglib___##type##_delete_recursive(type **tree, type *elem) {\
  ...
  res = 0;\
  if (t==NULL) {\
    SGLIB_ASSERT(0 && "The element to delete not found in the tree,  use 'delete_if_member'"!=NULL);\
  } else {\
    cmp = comparator(elem, t);\
    if (cmp < 0 || (cmp==0 && elem<t)) {\
      ...
      SGLIB_ASSERT(elem==t && "Deleting an element which is non member of the tree, use 'delete_if_member'"!=NULL);\
      if (t->left == NULL) {\
        if (t->right == NULL) {\
          /* a leaf, delete, it; */\
          *tree = NULL;\
...
static void sglib___##type##_consistency_check_recursive(type *t, int *pathdeep, int cdeep) {\
  if (t==NULL) {\
    ...
    else SGLIB_ASSERT(*pathdeep == cdeep);\
  } else {\
    if (t->left!=NULL) SGLIB_ASSERT(comparator(t->left, t) <= 0);\
    if (t->right!=NULL) SGLIB_ASSERT(comparator(t, t->right) <= 0);\
    if (SGLIB___GET_VALUE(t->bits) == RED) {\
      SGLIB_ASSERT(t->left == NULL || SGLIB___GET_VALUE(t->left->bits)==BLACK);\
      SGLIB_ASSERT(t->right == NULL || SGLIB___GET_VALUE(t->right->bits)==BLACK);\
...
void sglib___##type##_consistency_check(type *t) {\
  int pathDeep;\
  SGLIB_ASSERT(t==NULL || SGLIB___GET_VALUE(t->bits) == BLACK);\
  pathDeep = -1;\
